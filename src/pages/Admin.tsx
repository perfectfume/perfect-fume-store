...
import { Download, AlertTriangle, TrendingUp, Package, Users, Calendar, Filter, Inbox, Clock, Truck, CheckCircle, Pencil, X, Lock } from 'lucide-react'; // 🔥 Lock Icon Added
            ...
            <div className="bg-white/5 p-6 rounded-2xl border border-white/10 h-fit">
              ...
              <form onSubmit={handleAddProduct} className="space-y-4 text-sm">
                ...
                <div className="space-y-3 bg-black/30 p-3 rounded-lg border border-white/5">
                  ...
                  {isImageUploading && <p className="text-xs text-purple-400 font-bold animate-pulse">Uploading image to server...</p>}
            ...
            <form onSubmit={handleUpdateProduct} className="space-y-4 text-sm flex-1 overflow-y-auto pr-3 no-scrollbar">
              ...
              <div className="space-y-3 bg-black/30 p-3 rounded-lg border border-white/5">
                ...
                <div className="flex flex-col md:flex-row items-start md:items-center gap-2">
                  ...
                  <span className="text-gray-500 text-xs hidden md:block">OR</span>
                  <input type="file" accept="image/*" onChange={(e) => handleImageUpload(e, setEditImage)} className="text-xs w-full md:w-64 text-gray-400 file:mr-2 file:py-2 file:px-3 file:rounded-lg file:border-0 file:text-xs file:font-bold file:bg-blue-600 file:text-white cursor-pointer" />
                </div>
                {isImageUploading && <p className="text-xs text-blue-400 font-bold animate-pulse">Uploading image to server...</p>}
