...
const Navbar = () => {
  const { cart, isCartOpen, toggleCart, removeFromCart, getTotals, userEmail, userPhone, setUserAuth, logout } = useStore();
  const { total } = getTotals();
  const cartCount = cart.reduce((acc, item) => acc + item.quantity, 0);

  // Login Modal States
  const [isAuthOpen, setIsAuthOpen] = useState(false);
  const [loginStep, setLoginStep] = useState(1); // 1: Email+Phone, 2: OTP
  ...
  const handleVerifyOtp = async (e: any) => {
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok && data.success) {
        setUserAuth(emailInput, phoneInput); // Login Save
        setIsAuthOpen(false);
        setLoginStep(1);
        setOtpInput('');
  ...
  const handleLogout = () => {
    if(window.confirm("Sotti Logout korben?")) {
      logout();
    }
  };
